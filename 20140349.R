...
####
#filee = dir(pattern = "json")
#datas <- lapply(filee,fromJSON(filee, method="C"))
#finaldata = do.call(rbind,datas)

#####
j_filea <-"./talks_art.json"
j_filef <-"./talks_free.json"
j_filem <-"./talks_music.json"
j_filet <-"./talks_time.json"

###Read a json file to workspace
talk_art<-fromJSON(file=j_filea, method="C")
talk_free<-fromJSON(file=j_filef, method="C")
talk_music<-fromJSON(file=j_filem, method="C")
talk_time<-fromJSON(file=j_filet, method="C")
#str(talk_art)
#str(talk_free)
#str(talk_music)
#str(talk_time)
##saving column names
talk_names<-names(talk_music$talks[[1]]$talk)
talk_names
###Convertobject type
talk_art_list<-lapply(talk_art$talks, function(x){unlist(x)})
talk_free_list<-lapply(talk_free$talks, function(x){unlist(x)})
talk_music_list<-lapply(talk_music$talks, function(x){unlist(x)})
talk_time_list<-lapply(talk_time$talks, function(x){unlist(x)})
str(talk_music_list)
parse_talk_all<-data.frame()
##combining datasets
df_parse_talk_all<-do.call("rbind", c(parse_talk_all,talk_art_list,talk_free_list, talk_music_list,talk_time_list))
...
## removing repetition
total<-df_parse_talk_all[ !duplicated( df_parse_talk_all$id) , ] 
...
ted_docs <- Corpus(VectorSource(total$description))
class(ted_docs)
...
total_freq<-data.frame(stem_freq)
str(total_freq)
...
png("./dendrogram_tc_tfidf_average_sparse95.png", width = 1500, height=800)
plot(fit)
###k= number of clusters
rect.hclust(fit, k=10)
dev.off()
...
write.table(df_groups, "./tc_tfidf_average_0.95.txt", row.names=FALSE, col.names=TRUE, sep="\t")
